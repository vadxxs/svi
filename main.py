...
async def mirror_document_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.message or update.effective_user.id == ADMIN_ID:
        return
    ...
    try:
        ...
        log.exception("failed to mirror document to admin")


async def mirror_voice_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.message or update.effective_user.id == ADMIN_ID:
        return
    ...
    info = (
        f"🎤 Нове голосове повідомлення\n"
        f"ID: {user.id}\n"
        f"Username: @{user.username if user.username else '—'}\n"
        f"Name: {user.full_name}\n\n"
        f"Тривалість: {voice.duration} с"
    )
    ...
    try:
        ...
        log.exception("failed to mirror voice to admin")


async def mirror_video_note_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.message or update.effective_user.id == ADMIN_ID:
        return
    ...
    info = (
        f"🎬 Нове відеоповідомлення\n"
        f"ID: {user.id}\n"
        f"Username: @{user.username if user.username else '—'}\n"
        f"Name: {user.full_name}\n\n"
        f"Тривалість: {video_note.duration} с"
    )

    try:
        await context.bot.send_message(chat_id=ADMIN_ID, text=info)
        await context.bot.send_video_note(chat_id=ADMIN_ID, video_note=video_note.file_id)
    except Exception:
        log.exception("failed to mirror video_note to admin")


async def mirror_sticker_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.message or update.effective_user.id == ADMIN_ID:
        return
    ...
    try:
        ...
        log.exception("failed to mirror sticker to admin")


async def mirror_location_to_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if not update.message or update.effective_user.id == ADMIN_ID:
        return
    ...
    try:
        ...
        log.exception("failed to mirror location to admin")
...
def main() -> None:
    ...
    # Пересилання всіх типів повідомлень адміну
    ...
    app.add_handler(MessageHandler(filters.Document.ALL, mirror_document_to_admin), group=10)
    app.add_handler(MessageHandler(filters.VOICE, mirror_voice_to_admin), group=10)
    app.add_handler(MessageHandler(filters.VIDEO_NOTE, mirror_video_note_to_admin), group=10)
    app.add_handler(MessageHandler(filters.Sticker.ALL, mirror_sticker_to_admin), group=10)
    app.add_handler(MessageHandler(filters.LOCATION, mirror_location_to_admin), group=10)
